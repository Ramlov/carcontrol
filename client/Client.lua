...
ctrl.setEngine = function(d,s)
    ...
    if GetIsVehicleEngineRunning(v) then
        TriggerEvent("pNotify:SendNotification", source,{text = "Bilen er slukket", type = "error", queue = "global", timeout = 2500, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
        SetVehicleEngineOn(v, false, false)
    else
        TriggerEvent("pNotify:SendNotification", source,{text = "Bilen er tændt", type = "success", queue = "global", timeout = 2500, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
